...
namespace Capa_Datos
{
    public class Cotizaciones
    {
        ...
        public void insertarProductosCot(string nombre, string marca, string codigocot,double precio, int cantidad, double subtotal)
        {
            try
            {
                SqlCommand addProd = new SqlCommand("GenerarProductosCotizacion", conn.Connection);
                ...
                addProd.Parameters.AddWithValue("@cot", codigocot);
        ...
        public void insertarCotizacion(string codigo, string fecha, double subtotal, double total, string cliente, string correo, string vendedor)
        {
            try
            {

                SqlCommand addProd = new SqlCommand("GenerarCotizacion", conn.Connection);
                addProd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable verCotizaciones(string word)
        {
            try
            {
                SqlCommand lstcots = new SqlCommand("verCotizaciones", conn.Connection);
                lstcots.CommandType = CommandType.StoredProcedure;
                    lstcots.Parameters.AddWithValue("@word", word);
                conn.OpenConnection();

                DataTable dt = new DataTable();
                SqlDataAdapter da = new SqlDataAdapter(lstcots);
                da.Fill(dt);
